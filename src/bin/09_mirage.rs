...
fn main() {
    let file = fs::read_to_string("./inputs/09_mirage.txt").unwrap();
    let histories = file.lines();
    println!("{}", get_extrapolated_forward_sum(histories.clone()));
...
fn extrapolate_history_forward(values: impl Iterator<Item=isize>) -> isize {
    ...
    let next_values = values.iter()
        .tuple_windows()
        .map(|(left, right)| *right - *left)
        .collect::<Vec<isize>>();
    (if next_values.iter().all(|x| *x == 0) {
        0
    } else {
        extrapolate_history_forward(next_values.into_iter())
    }) + *values.last().unwrap()
}
...
fn get_extrapolated_forward_sum<'a>(histories: impl Iterator<Item=&'a str>) -> isize {
    ...
    histories.map(|history|
        extrapolate_history_forward(
            history.split_whitespace()
                .map(|val| isize::from_str(val).unwrap())
        )
    ).sum()
}
...
fn extrapolate_history_backward(values: impl Iterator<Item=isize>) -> isize {
    ...
    *values.first().unwrap() - (if next_values.iter().all(|x| *x == 0) {
        0
    } else {
        extrapolate_history_backward(next_values.into_iter())
    })
}
